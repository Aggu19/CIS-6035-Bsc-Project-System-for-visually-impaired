...
const Contacts: React.FC = () => {
  ...
  // Strictly use environment variables for EmailJS
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!EMAILJS_SERVICE_ID || !EMAILJS_TEMPLATE_ID || !EMAILJS_PUBLIC_KEY) {
        throw new Error('Email service is not configured. Please set VITE_EMAILJS_* env vars.');
      }
      // NOTE: Template variables must match your EmailJS template fields
      await emailjs.send(
        EMAILJS_SERVICE_ID,
        EMAILJS_TEMPLATE_ID,
        {
      ...
      // Common causes: invalid service/template/key, domain not allowed in EmailJS settings, or template vars mismatch
      setError(`Failed to send message: ${reason}`);
    } finally {
      setSending(false);
    }
  };
